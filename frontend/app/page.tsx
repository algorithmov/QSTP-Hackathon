  ...
  async function handleSubmit(payload: RouteRequest) {
    ...
    try {
      ...
      if (mediaFile) {
        ...
        if (mediaUrl) {
          finalPayload = { ...payload, media_url: mediaUrl };
        }
      }
      const response = await routeContent(finalPayload);
      setResult(response);
      setStatus("success");
    } catch {
      setError("Routing failed. Check the backend is running and try again.");
      setStatus("error");
    }
  }
        ...
        {result ? (
          <div className="grid gap-5 xl:grid-cols-[1fr_380px]">
            ...
            <aside className="space-y-5">
              ...
              <MenaMap data={mapData} routes={routes} />
            </aside>
          </div>
        ) : (
          <RouteBoard routes={[]} />
